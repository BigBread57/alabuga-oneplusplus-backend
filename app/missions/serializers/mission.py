...
class MissionCreateSerializer(serializers.ModelSerializer):
    ...
    competencies = serializers.ListField(child=serializers.DictField(), required=False, write_only=True)
    artifacts = serializers.ListField(child=serializers.DictField(), required=False, write_only=True)
    ...
    def create(self, validated_data):
        ...
        for comp_data in competencies:
            MissionCompetency.objects.create(
                mission=mission, competency_id=comp_data["competency_id"], points=comp_data.get("points", 1)
            )

        for art_data in artifacts:
            MissionArtifact.objects.create(
                mission=mission, artifact_id=art_data["artifact_id"], drop_chance=art_data.get("drop_chance", 1.0)
            )
